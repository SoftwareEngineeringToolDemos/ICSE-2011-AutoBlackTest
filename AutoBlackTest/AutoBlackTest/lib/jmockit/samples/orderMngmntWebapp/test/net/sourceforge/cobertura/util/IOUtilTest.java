...
public class IOUtilTest extends TestCase
{
   ...
   private static final byte[] singletonByteArray = {7};
   private static final byte[] smallByteArray = {1, 0, 2, -128, 127};
   ...
   {
      byte[] out = IOUtil.createByteArrayFromInputStream(new ByteArrayInputStream(smallByteArray));
      assertEquals(smallByteArray, out);

      out = IOUtil.createByteArrayFromInputStream(new ByteArrayInputStream(emptyByteArray));
      assertEquals(emptyByteArray, out);

      out = IOUtil.createByteArrayFromInputStream(new ByteArrayInputStream(singletonByteArray));
      assertEquals(singletonByteArray, out);

      byte[] bigArray = generateBigByteArray();
      out = IOUtil.createByteArrayFromInputStream(new ByteArrayInputStream(bigArray));
   ...
   {
      assertNull(IOUtil.closeInputStream(new ByteArrayInputStream(new byte[0])));
   }

   public void testCloseOutputStream()
   {
      assertNull(IOUtil.closeOutputStream(new ByteArrayOutputStream(10)));
   }

   public void testGetPrintWriter(FileOutputStream mock) throws Exception
   {
      assertNotNull(IOUtil.getPrintWriter(new File("test")));
   }
}
