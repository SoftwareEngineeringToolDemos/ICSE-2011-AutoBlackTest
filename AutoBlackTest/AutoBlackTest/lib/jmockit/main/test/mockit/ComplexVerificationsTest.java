...
public class ComplexVerificationsTest
{
   ...
   public void usingStrictExpectationsMostly(final A a, final B b, final C c)
   {
      new Expectations() {{
         ...
         // Meets requirements 3b and 4, but NOT 3a:
         c.bar(); notStrict(); times = input.length;
         b.foo(); notStrict(); times = input.length;
      ...
      // Requirement 6 is met since all classes are mocked strictly,
      // even if some individual expectations are non-strict.
   }
